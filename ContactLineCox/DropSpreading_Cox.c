 ...
 * Macroscopic contact angle is set in the first grid cell at the left boundary --> Using the Cox-Voinov model that introduces a fitting parameter, M_c.
 * Rest of the dynamics away from the first cell is set to evolve based on the Navier-Stokes equations without any other assumption. 
 * We call this method: Contact line + Cox
*/
...
double Ohw, Oha, hf, tmax, Ldomain, delta, theta0, M_c;
int MAXlevel;
...
int main(int argc, char const *argv[]) {
  ...
  M_c = 2*Ohw; // fitting parameter for the Cox-Voinov model
