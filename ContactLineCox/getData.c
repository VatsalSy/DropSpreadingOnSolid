...
char filename[80];
int nx, ny, len;
double xmin, ymin, xmax, ymax, Deltax, Deltay, Ohw, Oha, mu1, mu2, theta0, M_c;
scalar D2c[], vel[];
scalar * list = NULL;
...
int main(int a, char const *arguments[])
{
  ...
  ny = atoi(arguments[6]);
  Ohw = atof(arguments[7]);
  Oha = atof(arguments[8]);
  theta0 = 15.0;

  // fprintf(ferr, "xmin %g, ymin %g, xmax %g, ymax %g, ny %d, Ohw %g, Oha %g\n", xmin, ymin, xmax, ymax, ny, Ohw, Oha);
